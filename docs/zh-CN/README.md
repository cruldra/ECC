...
### v2.2.39 — 修安装解析：target 不支持的依赖不再拖垮父模块，codex 之前每次都静默少装 76 个技能（2026年9月）
...
### v2.2.36 — 四道闸门图改用 mermaid，控制台和 GitHub 都能渲染出来（2026年9月）

### v2.2.35 — 原型三段定稿：抬头 / 现状 · 解决方案 / 交互演示工作区，屏幕上不许出现问号（2026年9月）
...
### v2.2.25 — 收录 closing-custom-dev-deals（承接定制开发：四道闸门 / 议价护栏 / 合同条款），公司数字加密存放（2026年9月）

### v2.2.24 — 控制台译本视图的 YAML 头改成键值表（2026年9月）

### v2.2.23 — 控制台地址栏记历史：进编辑器、切类型都能用浏览器后退 / 触控板手势退回（2026年9月）

### v2.2.22 — 收录 lightweb-fastapi-starter；控制台前端拆成 store + 面板组件 + 模板片段（2026年9月）

### v2.2.21 — 控制台静态文件带内容哈希，改完不再吃浏览器旧缓存（2026年9月）

### v2.2.20 — 删掉上游机器翻译的 agent 镜像（199 个文件），改为控制台按需翻译并记 source_hash（2026年9月）
...
**搞定！** 你现在可以使用 68 个智能体、298 项技能和 94 个命令了。
...
| 技能 | PASS: 298 项   | PASS: 37 项 | **Claude Code 领先** |
